...
class Complex(object):
    ...
    def __mul__(self, no):
        product_real = self.real * no.real - self.imaginary * no.imaginary
        product_img = self.real * no.imaginary + no.real * self.imaginary
        return Complex(product_real, product_img)
    
    def __truediv__(self, no):
        denominator = no.real**2 + no.imaginary**2
        if denominator == 0:
            raise ZeroDivisionError("can't be divided")
        div_real = (self.real * no.real + self.imaginary * no.imaginary) / denominator
        div_img = (self.imaginary * no.real - self.real * no.imaginary) / denominator
        return Complex(div_real, div_img)
