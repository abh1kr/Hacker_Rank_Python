...
# CODE:
k=int(input())
list_family_room=list(map(int,input().split()))
set_con=set(list_family_room)
captain_room={}
for i in list_family_room:
    captain_room[i]=captain_room.get(i,0)+1
for i ,count in captain_room.items():
    if count==1:
        print(i)
